 ...
 */
public class RtspSetupAction extends RtspAction {
	...
	public void execute(RtspMessage msg) {
		...
		ui.log("SETUP : " + resp.GetDescribe() + "\n");
		if (resp.GetStatus().equals("200")) {
			ui.showButton(false, true, true, true, true);
			String session = resp.GetSession();
			String token[] = session.split(";");
			client.setSession(token[0]);
		} else if (resp.GetStatus().equals("300")) {		
			client.redirect(resp.GetHeader("Location"));
		} else {
